...
// --- MOCK UTILITIES ---
import { fetchWordsOnce } from "../FIreStore"; // Adjust path as needed
import { addSolvedWord } from "../utils/storage"; // Adjust path as needed
...
// We mock children to control their behavior and verify props
jest.mock("@/components/Grass", () => "GrassMock");
jest.mock("@/components/HeadphoneButton", () => "HeadphoneButtonMock");
...
// Mock the Input component to allow simulating a guess action,
// passing the critical state setters so the test can simulate the game loop.
const MockInput = ({ setSolvedWord, setWrongGuesses, letters, solvedWord, WORD, ...props }: any) => {
    const React = require("react");
    // This button will simulate a correct guess
    const handleCorrectGuess = () => {
        // Find the index of the first not guessed letter in the word (e.g., 'H' in 'HANGMAN')
        const firstUnsolvedLetter = WORD.split("").find((l: string) => !letters.includes(l));
        if (firstUnsolvedLetter) {
            setSolvedWord(WORD.toUpperCase()); // Simulating a win by solving the word
        }
    };
    // This button will simulate an incorrect guess
    ...
    return (
        <React.Fragment>
            <require("react-native").Button title="GuessCorrect" onPress={handleCorrectGuess} testID="guess-correct-btn" />
            <require("react-native").Button title="GuessIncorrect" onPress={handleIncorrectGuess} testID="guess-incorrect-btn" />
        </React.Fragment>
    );
};
...
// Mock the WinOrLose modal to control end-game actions
const MockWinOrLose = ({ modalVisible, wrongGuesses, toHome, continueOrRetry }: any) => {
    const React = require("react");
    ...
    return (
        <require("react-native").View testID="win-or-lose-modal">
            <require("react-native").Text>{title}</require("react-native").Text>
            <require("react-native").Button title="To Home" onPress={toHome} testID="to-home-btn" />
            <require("react-native").Button title="Continue/Retry" onPress={continueOrRetry} testID="continue-retry-btn" />
...
describe("GamePage (Hangman) Core Functionality", () => {
  ...
  // Since the MockInput simulates a win on 'GuessCorrect', 
  // we will test the win condition directly in the next section.
  ...
  it("should reset the game state when 'Continue/Retry' is pressed", async () => {
    ...
    // 3. Verify a new word was attempted to be fetched (or state updated, indicated by roundKey change)
    // The easiest way to verify a reset is confirming `fetchWordsOnce` was called twice
  ...
  it("should navigate to home screen when 'To Home' is pressed", async () => {
    ...
    // 1. Verify navigation to the home page (use router.replace for a clean exit)
